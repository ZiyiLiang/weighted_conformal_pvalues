...
make.plot.shift <- function(n_in.plot, outlier.group) {
    ...
    pp <- results.fdr %>%
        ...
        ylab("") +
        theme_bw()
    pp %>% ggsave(file=sprintf("figures/experiment_real_animals_shift_%s_nin_%s.pdf", outlier.group, n_in.plot), width=6, height=3, units="in")
}

make.plot.shift(1000, 1)
make.plot.shift(1000, 2)
make.plot.shift(1000, 3)

make.plot.shift(7824, 1)
make.plot.shift(7824, 2)
make.plot.shift(7824, 3)
