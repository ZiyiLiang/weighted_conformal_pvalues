...
# Output file #
###############
outfile_prefix = "results/setup_shift" + str(setup) + "/" +str(data_name) + "_n"+str(n) + "_p" + str(p) + "_a" + str(a) + "_as" + str(a_shift) + "_purity" + str(purity) + "_seed" + str(random_state)
...
def run_experiment(dataset, random_state):
    ...
    results_tmp["Model"] = "Ensemble"
    results_tmp["E_U1_Y0"] = np.nan
    results_tmp["E_U1_Y0_aprox"] = np.nan
    results_tmp["1/log(n1+1)"] = np.nan
    results = pd.concat([results, results_tmp])
