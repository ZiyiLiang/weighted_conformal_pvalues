...
if [[ $SETUP == 1 ]]; then
  ...
  P_LIST=(1000)
  A_LIST=(0.7)
  A_SHIFT_LIST=(0.5 -0.5)
  PURITY_LIST=(0.5 0.75 0.9)
#  PURITY_LIST=(0.5)
  GAMMA_LIST=(1.0 0.1 0.01 0.005 0.002 0.001 0.0001 0.00001 0.000001)
#  GAMMA_LIST=(0.000001)
  SEED_LIST=$(seq 1 25)
...
LOGS="logs"
mkdir -p $LOGS
mkdir -p $LOGS"/setup_power_shift"$SETUP

OUT_DIR="results"
mkdir -p $OUT_DIR
mkdir -p $OUT_DIR"/setup_power_shift"$SETUP
          ...
          for A_SHIFT in "${A_SHIFT_LIST[@]}"; do
            for PURITY in "${PURITY_LIST[@]}"; do
              for GAMMA in "${GAMMA_LIST[@]}"; do
                ...
                JOBN="setup_power_shift"$SETUP"/"$DATA"_n"$N"_p"$P"_a"$A"_as"$A_SHIFT"_purity"$PURITY"_gamma"$GAMMA"_seed"$SEED
                ...
                if [[ $COMPLETE -eq 0 ]]; then
                  # Script to be run
                  SCRIPT="experiment_power_shift.sh $SETUP $DATA $N $P $A $A_SHIFT $PURITY $GAMMA $SEED"
